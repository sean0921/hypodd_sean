...
c Num REc routine; altered: takes median from abs(X); fw
cfw..
c      implicit none
c      integer*4 maxdata,n,np,nm,j
c      real x(maxdata),xmed
c      real a, eps,ap,sum,sumx,xp,xm,xx,dum,aa,big,afac,amp,am
cfw

      DIMENSION X(N)
      PARAMETER (BIG=1.E30,AFAC=1.5,AMP=1.5)
      A=0.5*(abs(X(1))+abs(X(N)))
      EPS=ABS(abs(X(N))-abs(X(1)))
      AP=BIG
      AM=-BIG
1     SUM=0.
      SUMX=0.
